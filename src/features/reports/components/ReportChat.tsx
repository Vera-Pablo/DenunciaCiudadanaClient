...
export const ReportChat: React.FC<ReportChatProps> = ({
  ...
  const handleSend = () => {
    ...
    const optimistic: Comment = {
      ...
      user: {
        name: user?.name ?? "",
        role: { type_role: user?.role?.type_role ?? "Usuario" },
      },
    };
          ...
          [...allComments].reverse().map((comment) => {
            ...
            return (
              ...
              >
                ...
                >
                  <p
                    className={`text-[10px] font-semibold mb-1 ${own ? "text-on-primary/80" : "text-slate-500"}`}
                  >
                    {comment.user?.name || "Tú"} ·{" "}
                    {comment.user?.role?.type_role || "Ciudadano"}
                  </p>
                  <p className="whitespace-pre-wrap leading-relaxed">
                    {comment.text}
                  </p>
                  <p
                    className={`text-[10px] mt-1 text-right ${own ? "text-on-primary/60" : "text-slate-400"}`}
                  >
